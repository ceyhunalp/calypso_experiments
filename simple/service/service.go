...
const ServiceName = "SimpleCalypsoService"
...
func init() {
	...
	var err error
	templateID, err = onet.RegisterNewService(ServiceName, newSimpleCalypsoService)
	log.ErrFatal(err)
...
// than one structure.
var storageID = []byte("SimpleCalypso")
...
func newSimpleCalypsoService(c *onet.Context) (onet.Service, error) {
	log.Print("In NewSimpleCalypsoService")
	db, bucket := c.GetAdditionalBucket([]byte("simplecalypsotransactions"))
	s := &Service{
		ServiceProcessor: onet.NewServiceProcessor(c),
		db:               NewSimpleCalypsoDB(db, bucket),
	}
	...
	log.Print("Returning from NewSimpleCalypsoService")
	return s, nil
}
